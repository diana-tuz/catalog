...
export const CatalogPage: FC = () => {
  ...
  const [currentList, setCurrentList] =
    useState<ProductCardPropsType[]>(listOfGoods)
  const [preparedList, setPreparedList] =
    useState<ProductCardPropsType[]>(listOfGoods)
  ...
  useEffect(() => {
    ...
    if (sortBy === 'Newest') {
      ...
      sortedList = sortedList.sort((item1, item2) =>
  ...
  const pagesArray = Array.from(
    ...
    (_, i) => i + 1,
  ).slice(pagesStartIndex, pagesStartIndex + 8)
  ...
  const handleTabsNext = () => {
    ...
    if (currentPage - 8 === pagesStartIndex) {
      setPagesStartIndex(pagesStartIndex + 8)
    }
  }
  ...
  return (
    <Container>
      ...
      <Settings>
        <SelectBlock>
          <Label>{'Sort by'}</Label>
          <Select onChange={(e) => setSortBy(e.target.value as Category)}>
            {category.map((item) => (
